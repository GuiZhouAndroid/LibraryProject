...
//                .compose(getBindLifecycleFlowableTransformer()) //加上这行代码，视图关闭调用onDestroy()定时结束，反之继续打印aLong值
...
//        });
        ...
        mainModel.login2().compose(HandlerTransformer.getFlowableTransformerScheduler(getBindLifecycleFlowableTransformer())).subscribe(new BaseFlowableSubscriber<My>(App.getContext(), BaseActivity.getNowActivity(),9) {
                    @Override
